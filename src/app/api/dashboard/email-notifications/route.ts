...
function buildEmailHtml({
  ...
  const color = isCancellation ? "#ef4444" : "#0ea5e9";
      ...
      <table width="100%" style="max-width:520px;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.1)">
        ...
        <tr><td style="padding:28px 32px">
          ...
          <div style="text-align:center;margin-top:24px">
            <a href="https://idental.app/${shopSlug}" style="background:${color};color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px">Ver mi reserva</a>
          </div>` : ""}
        </td></tr>
        <tr><td style="padding:16px 32px;border-top:1px solid #f3f4f6;text-align:center">
          <p style="margin:0;color:#9ca3af;font-size:12px">Powered by <a href="https://idental.app" style="color:#0ea5e9;text-decoration:none">iBarber</a></p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function POST(request: Request) {
  ...
  if (recipientEmail && process.env.RESEND_API_KEY) {
    try {
      ...
      const barberName = (booking.barbers as unknown as { display_name: string } | null)?.display_name || "Tu dentista";
