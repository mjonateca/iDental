...
import { ArrowLeft, CheckCircle2, Clock, Home, Loader2, Scissors, UserRound, Users } from "lucide-react";
            ...
            {selectedDate && (
              <div>
                ...
                {selectedDayConfig?.closed ? (
                  <p className="rounded-lg border bg-background p-4 text-sm text-muted-foreground">La clínica dental está cerrada ese día.</p>
                ) : loadingSlots ? (
                ...
                {shop.payments_enabled && (
                  <div className="rounded-lg bg-muted p-3 text-sm text-muted-foreground">
                    {shop.online_payment_mode === "required"
                      ? "Esta clínica dental tiene pago online obligatorio. Después de reservar podrás completar el pago desde tu panel."
                      : "Esta clínica dental admite pago online. Después de reservar podrás pagar desde tu panel si quieres dejarla abonada."}
                  </div>
                )}
              </CardContent>
            </Card>
