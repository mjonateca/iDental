    ...
    <div className="max-w-6xl p-4 md:p-8">
      ...
      {currentTab === "summary" && (
        <Card className="shadow-none">
          <CardHeader>
            <CardTitle>Clínicas dentales cerca de tu zona</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2">
            {shops.length === 0 ? (
              <p className="text-sm text-muted-foreground">No hay clínicas dentales activas en tu ciudad.</p>
            ) : (
              shops.map((shop) => (
                <div key={shop.id} className="rounded-lg border p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <Button asChild size="sm" variant="outline">
                      <Link href={`/${shop.slug}`}>Ver clínica dental</Link>
                    </Button>
                  </div>
                </div>
              ))
            ...
            {bookings.length === 0 ? (
              ...
              bookings.map((booking) => (
                <div key={booking.id} className="rounded-lg border p-4">
                  <p className="font-medium">{booking.shops?.name || "Clínica dental"}</p>
                  ...
                  {booking.shops?.slug && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      <Button asChild size="sm" variant="outline">
                        <Link href={`/${booking.shops.slug}`}>Ver clínica dental</Link>
                      </Button>
                      <Button asChild size="sm">
      ...
      {currentTab === "favorites" && (
        <div className="grid gap-5 lg:grid-cols-2">
          <Card className="shadow-none">
            <CardHeader>
              <CardTitle>Clínicas dentales favoritas</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              {favoriteShops.length === 0 ? (
                <p className="text-sm text-muted-foreground">Todavía no has marcado clínicas dentales favoritas.</p>
              ) : (
