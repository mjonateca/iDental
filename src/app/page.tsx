  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5 group">
            <div
              ...
              style={{
                background:
                  "linear-gradient(135deg, hsl(201,85%,46%), hsl(198,82%,62%))",
              }}
            >
          ...
          <nav className="flex items-center gap-2">
            ...
            <Button
              ...
              style={{
                background:
                  "linear-gradient(135deg, hsl(201,85%,48%), hsl(198,82%,64%))",
              }}
            >
      ...
      <section
        ...
        style={{
          background:
            "linear-gradient(135deg, hsl(210,72%,16%) 0%, hsl(204,72%,24%) 42%, hsl(197,76%,32%) 100%)",
        }}
      >
        <div
              ...
              <div className="flex flex-wrap gap-x-5 gap-y-2 mb-10">
                {[
                  ...
                  <div key={text} className="flex items-center gap-1.5 text-white/65 text-sm">
                    <CheckCircle2 className="h-4 w-4 flex-shrink-0" style={{ color: "hsl(198, 96%, 82%)" }} />
                    {text}
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white/8 backdrop-blur-xl border border-white/15 rounded-2xl p-4 shadow-2xl">
                ...
                <form className="flex flex-col sm:flex-row gap-2.5">
                  ...
                  <button
                    ...
                    style={{ background: "hsl(198, 92%, 66%)", color: "#0d1117" }}
                  >
                ...
                <div className="rounded-2xl border border-white/15 bg-white/8 backdrop-blur-xl p-6 shadow-2xl">
                  <div className="flex items-center gap-3 mb-5">
                    <div
                      ...
                      style={{ background: "linear-gradient(135deg, hsl(201,85%,46%), hsl(198,82%,62%))" }}
                    >
                      B
                    </div>
                    <div>
                      <p className="text-white font-bold text-sm">Clínica dental Premium</p>
                  ...
                  <div className="space-y-2.5 mb-5">
                    {[
                      { name: "Limpieza dental", price: "$350", time: "30 min" },
                      { name: "Valoración general", price: "$550", time: "45 min" },
                      { name: "Blanqueamiento", price: "$750", time: "60 min" },
                    ].map((s) => (
                      <div key={s.name} className="flex items-center justify-between rounded-lg border border-white/10 bg-white/5 px-3.5 py-2.5">
                        ...
                        <div className="text-right">
                          <p className="text-xs font-bold" style={{ color: "hsl(198,96%,82%)" }}>{s.price}</p>
      ...
      <section className="border-b bg-card">
        <div className="max-w-6xl mx-auto px-4 py-5 grid grid-cols-3 divide-x divide-border">
          {[
            { icon: Sparkles, value: shops.length > 0 ? `${shops.length}+` : "10+", label: "Clínicas dentales activas" },
            { icon: Users, value: "500+", label: "Clientes atendidos" },
            { icon: Clock, value: "24/7", label: "Reservas online" },
          ].map(({ icon: Icon, value, label }) => (
            <div key={label} className="text-center px-4 py-1">
              <div className="flex items-center justify-center gap-2 mb-0.5">
                <Icon className="h-4 w-4" style={{ color: "hsl(201,85%,46%)" }} />
                <p className="text-2xl md:text-3xl font-black" style={{ color: "hsl(201,85%,46%)" }}>
                  {value}
                </p>
              </div>
      ...
      <section className="py-20 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: "hsl(201,85%,46%)" }}>
              Proceso simple
            </p>
            <h2 className="text-3xl md:text-4xl font-black text-foreground">¿Cómo funciona?</h2>
            <p className="text-muted-foreground mt-3 max-w-md mx-auto">
              Reserva tu próximo tratamiento en menos de 2 minutos
            </p>
          </div>

          <div className="grid gap-6 md:grid-cols-3">
            {[
              { step: "01", icon: Search, title: "Busca tu clínica dental", desc: "Filtra por ciudad y encuentra la clínica dental perfecta cerca de ti." },
              ...
              { step: "03", icon: Star, title: "Disfruta tu tratamiento", desc: "Recibe confirmación al instante y llega a tu cita sin esperas." },
              ...
              <div key={step} className="relative rounded-2xl border bg-card p-7 hover:border-primary/30 hover:shadow-lg transition-all duration-300 group">
                <div className="flex items-start justify-between mb-5">
                  <div
                    ...
                    style={{ background: "hsl(198,100%,97%)", color: "hsl(201,85%,46%)" }}
                  >
                    <Icon className="h-6 w-6" />
                  </div>
                  <span className="text-5xl font-black select-none opacity-8 group-hover:opacity-15 transition-opacity" style={{ color: "hsl(201,85%,46%)" }}>
                    {step}
                  </span>
                </div>
                <h3 className="font-bold text-lg mb-2 text-foreground">{title}</h3>
                <p className="text-muted-foreground text-sm leading-relaxed">{desc}</p>
                <div className="mt-4 flex items-center gap-1 text-xs font-semibold opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: "hsl(201,85%,46%)" }}>
      ...
      {/* LISTINGS */}
      <section className="py-14 px-4" style={{ background: "hsl(198,100%,98%)" }}>
        <div className="max-w-6xl mx-auto">
          <div className="flex items-end justify-between gap-4 mb-10">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: "hsl(201,85%,46%)" }}>
                Disponibles ahora
              </p>
              <h2 className="text-2xl font-black text-foreground">
                {selectedCity
                  ? `Clínicas dentales en ${selectedCity}`
                  : selectedCountry
                    ? `Clínicas dentales en ${COUNTRIES.find((country) => country.code === selectedCountry)?.name || "tu país"}`
            ...
            <Link
              ...
              style={{ color: "hsl(201,85%,46%)" }}
            >
              Registrar mi clínica dental <ArrowRight className="h-4 w-4" />
            </Link>
          </div>

          {shops.length === 0 ? (
            <div className="rounded-2xl border bg-card p-14 text-center shadow-sm">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: "hsl(198,82%,92%)" }}>
                <Sparkles className="h-8 w-8" style={{ color: "hsl(201,85%,46%)" }} />
              </div>
              <p className="font-bold text-lg mb-1">No hay clínicas dentales activas aquí</p>
              <p className="text-sm text-muted-foreground mb-5">Prueba con otro país, otra ciudad o regresa pronto.</p>
              <Link href="/register" className="inline-flex items-center gap-1.5 text-sm font-bold hover:underline underline-offset-4" style={{ color: "hsl(201,85%,46%)" }}>
                ¿Tienes una clínica dental? Regístrala gratis →
              </Link>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ background: "linear-gradient(135deg, hsl(210,72%,18%) 0%, hsl(204,76%,28%) 50%, hsl(198,82%,40%) 100%)" }}
                    >
                      <div
                      ...
                      <div className="absolute bottom-0 left-5 translate-y-1/2">
                        <div
                          ...
                          style={{ background: "linear-gradient(135deg, hsl(201,85%,44%), hsl(198,82%,60%))" }}
                        >
                          {initials}
                        </div>
                      </div>
      ...
      <section className="py-20 px-4 bg-card">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {[
              { icon: Zap, title: "Reserva en segundos", desc: "Sin llamadas, sin WhatsApp. Elige hora disponible y confirma al instante.", accent: "hsl(201,85%,46%)", bg: "hsl(198,100%,97%)" },
              { icon: Shield, title: "Sin comisiones", desc: "No cobramos comisión por cita. El 100% del pago va directo a la clínica dental.", accent: "hsl(201,85%,46%)", bg: "hsl(198,100%,97%)" },
              { icon: TrendingUp, title: "Gestión inteligente", desc: "Las clínicas dentales gestionan su agenda, reducen no-shows y hacen crecer su negocio.", accent: "hsl(201,85%,46%)", bg: "hsl(198,100%,97%)" },
      ...
      <section className="py-16 px-4">
        <div className="max-w-6xl mx-auto">
          <div
            ...
            style={{ background: "linear-gradient(135deg, hsl(210,72%,16%) 0%, hsl(204,74%,24%) 58%, hsl(198,78%,34%) 100%)" }}
          >
            <div
            ...
            />
            <div className="absolute top-0 right-0 w-96 h-96 rounded-full opacity-20 blur-3xl pointer-events-none" style={{ background: "hsl(198, 88%, 74%)" }} />
            <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-10">
              <div className="max-w-lg">
                ...
                <h2 className="text-3xl md:text-4xl font-black text-white mb-4 leading-tight">
                  ...
                  <span style={{ color: "hsl(198, 96%, 82%)" }}>Regístrala gratis.</span>
                </h2>
                ...
                <div className="flex flex-wrap gap-4 mt-5">
                  {["Agenda digital", "Pagos online", "Panel de análisis"].map((f) => (
                    <div key={f} className="flex items-center gap-1.5 text-white/65 text-sm">
                      <CheckCircle2 className="h-4 w-4" style={{ color: "hsl(198,96%,82%)" }} />
                      {f}
                    </div>
                  ))}
                </div>
      ...
      {/* FOOTER */}
      <footer className="border-t py-10 px-4" style={{ background: "hsl(198,100%,98%)" }}>
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-8">
            <div>
              <div className="flex items-center gap-2.5 mb-2">
                <div className="rounded-lg p-1.5" style={{ background: "linear-gradient(135deg, hsl(201,85%,46%), hsl(198,82%,62%))" }}>
                  <LogoMark className="h-4 w-4 text-white" />
                </div>
                <span className="font-black text-base text-foreground">iDental</span>
              </div>
              <p className="text-xs text-muted-foreground max-w-xs leading-relaxed">
                La plataforma de reservas para clínicas dentales que quieren captar y gestionar citas online.
              </p>
            </div>
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  {[
                    ...
                    { href: "/register", label: "Registrar clínica dental" },
                  ].map((link) => (
          ...
          <div className="border-t pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-xs text-muted-foreground">
              © {new Date().getFullYear()} iDental · Plataforma de reservas para clínicas dentales
            </p>
